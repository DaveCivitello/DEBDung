...
# Use this function to grab the best fit from across all the chains in a previously saved run
...
generate_pars_list = function(pars){
  LL = full.likelihood.trans(pars)$log.density
  buffer = 1.05 # Corresponds to 500 LL units
  LL_check = (buffer+1)*LL
  attempts = 1
  # Transform and back transform to deal with the log-transformed density dependence parameters
  pars = DEB_parameter_trans(pars)
  while(LL_check < buffer*LL){
    disp = 0.05
    scale = rlnorm(n=length(pars), meanlog=0, sdlog=disp)
    rand_pars = pars*scale
    rand_pars[c("yPE", "yEF", "yEF2", "yEF3", "yED", "yRP", "yVE", "mP", "eh", "k", "fe")] = pmax(pmin(rand_pars[c("yPE", "yEF", "yEF2", "yEF3", "yED", "yRP", "yVE", "mP", "eh", "k", "fe")], 0.98), 1e-3)
    rand_pars = DEB_parameter_backtrans(rand_pars)
    LL_check = full.likelihood.trans(rand_pars)$log.density
    attempts = attempts + 1
    if(attempts %% 5 == 0){disp = disp*0.9} # reduce dispersion by 10% if not getting useful combos
  }
...
var_matrix = function(pars){
  matr = diag(length(pars))*1e-4*pars^2
  diag(matr)[which(names(pars) %in% c("ph", "alpha", "iPM", "EM", "DR", "Fh", "muD", "kR", "delta0", "hdelta", "theta", "mR", "hb", "rho", "kR2", "mR2", "kk2", "d02", "kkM", "d0M"))] = 0.01 # This corrects the jump size for log-transed parameters
  matr
}


MCMC.parallel2 <- function (p, n, init, n.chain = 8, n.cpu, packages = NULL, dyn.libs = NULL, 
...
# Bring in the best parameters
samps = readRDS("Dung_fitting_kappa_pl.Rda")
...
# Run the model and track the time
a = Sys.time() # Estimate is 67 hours to run 250,000 iterations [~60 iterations per chain per minute]
print(paste0("starting MCMC at ", a))

p2 = MCMC.parallel2(p=full.likelihood.trans, n=250000, init=pars, scale=variances, adapt=50000, n.chain=8, n.cpu = 8, 
                    packages=c("deSolve"), dyn.libs=c("IndividualModel_Shrink_P_dilute.so", "SizeCompModel_Shrink.so", "DEBDung_MoA_kappa.so"), 
		    acc.rate=0.3, list=T)
b = Sys.time()

b-a

saveRDS(p2, file="Dung_fitting_kappa_pl.Rda")