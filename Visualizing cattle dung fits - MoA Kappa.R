...
try(dyn.unload("IndividualModel_Shrink_P_dilute.so")) # unload so
...
get_best_fit = function(chain.list){
  ...
  list(
    chain.list[[which.max(chain.scores)]]$samples[
      which.max(chain.list[[which.max(chain.scores)]]$log.p), ],
    chain.list[[which.max(chain.scores)]]$cov.jump
  )
}

object = readRDS("Dung_fitting_kappa_pl.Rda")
best_fit = get_best_fit(object)
...
DEB_dung_vis = function(pars){
  ...
  dung40 <- data.frame(lsoda(inits, 0:dur.D, func = "derivs", dllname = "DEBDung_MoA_kappa",
                              initfunc = "initmod",  nout=2, outnames=c("Survival", "LG"), maxsteps=5e5,
                              params.t[1:34],  rtol=1e-6, atol=1e-6,
                              events = list(data = Dung.events(initial.food = 10.76, initial.dung = as.numeric(inits["Dung"])))))
  dung40
  ...
  dung80 <- data.frame(lsoda(inits, 0:dur.D, func = "derivs", dllname = "DEBDung_MoA_kappa",
                              initfunc = "initmod",  nout=2, outnames=c("Survival", "LG"), maxsteps=5e5,
                              params.t[1:34],  rtol=1e-6, atol=1e-6,
                              events = list(data = Dung.events(initial.food = 10.76, initial.dung = as.numeric(inits["Dung"])))))
  dung80
  ...
  dung120 <- data.frame(lsoda(inits, 0:dur.D, func = "derivs", dllname = "DEBDung_MoA_kappa",
                              initfunc = "initmod",  nout=2, outnames=c("Survival", "LG"), maxsteps=5e5,
                              params.t[1:34],  rtol=1e-6, atol=1e-6,
                              events = list(data = Dung.events(initial.food = 10.76, initial.dung = as.numeric(inits["Dung"])))))
  dung120
  ...
  dung160 <- data.frame(lsoda(inits, 0:dur.D, func = "derivs", dllname = "DEBDung_MoA_kappa",
                              initfunc = "initmod",  nout=2, outnames=c("Survival", "LG"), maxsteps=5e5,
                              params.t[1:34],  rtol=1e-6, atol=1e-6,
                              events = list(data = Dung.events(initial.food = 10.76, initial.dung = as.numeric(inits["Dung"])))))
  dung160
  ...
  dung200 <- data.frame(lsoda(inits, 0:dur.D, func = "derivs", dllname = "DEBDung_MoA_kappa",
                           initfunc = "initmod",  nout=2, outnames=c("Survival", "LG"), maxsteps=5e5,
                           params.t[1:34],  rtol=1e-6, atol=1e-6,
                           events = list(data = Dung.events(initial.food = 10.76, initial.dung = as.numeric(inits["Dung"])))))
  dung200
