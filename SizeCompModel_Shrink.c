...
/* Derivatives and 4 output variables */
...
double LC = y[9];
double LGC = fmax(y[9], yout[3]);
double VOLC = pow(LC,3);
double SAC = pow(LC,2);
...
double aM = iM*fmax(yEF, fmax(yEF2, yEF3));
...
double gc = 1/(yVE*k*EM);
double Jecc = y[10]*gc/(gc + y[10])*(aM*SAC + yVE*EM*(mV+mR*EM*y[13])*Chi*VOLC);

ydot[9] = yVE/(3*Chi*SAC)*(k*Jecc - (mV+mR*EM*y[13])*Chi*VOLC);
ydot[10] = aM/(Chi*EM*LC)*(fH - y[10]);
if(y[11] < DR){
  ydot[11] = (1 - k)*Jecc - mD*y[11];
  ydot[12] = 0;}else{
  ydot[11] = fmin(0, (1 - k)*Jecc - mD*DR);
  ydot[12] = fmax((1 - k)*Jecc - mD*DR, 0);}
  ydot[13] = kR*(1-y[9]) - kR*y[13] - 3*y[13]*ydot[9]/LC;
  ydot[14] = kk*fmax(y[13] - d0, 0) + hb;
...
/* END file SizeCompModel_Shrink.c */ 
